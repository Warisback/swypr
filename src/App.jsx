...
function Shortcuts({ onClose }) {
  ...
  return <dialog ref={dialog} className="shortcut-dialog" onCancel={event => { event.preventDefault(); onClose(); }} onClick={event => { if (event.target === dialog.current) onClose(); }}><div className="shortcut-head"><h2>a little backstage magic.</h2><button autoFocus aria-label="Close shortcuts" onClick={onClose}><X size={20}/></button></div><p><kbd>L</kbd> bring in a new message</p><p><kbd>R</kbd> reset the scene</p><p><kbd>G</kbd> put Glass Drop first</p><p><kbd>?</kbd> these shortcuts</p><p><kbd>Esc</kbd> back to your inbox</p></dialog>;
}
export default function App() {
  ...
  useEffect(() => {
    const handler = event => {
      ...
      if (event.key.toLowerCase() === 'r') { event.preventDefault(); inbox.resetDemo(); setView('queue'); }
  ...
  return <MotionConfig reducedMotion="user"><div className="desktop-stage"><aside className="desktop-left" aria-hidden="true"><div className="note-line"/><p>4,800 dms<br/>a month. <em>mostly<br/>the same question.</em></p><div className="ghost-card gc1"><span className="ghost-avatar">e</span><div><strong>@ellie.mp</strong><span>what shade are u wearing in todays video???</span></div></div><div className="ghost-card gc2"><span className="ghost-avatar">p</span><div><strong>@priyagets.ready</strong><span>do i need the barrier cream if i already got the night serum</span></div></div><div className="ghost-card gc3"><span className="ghost-avatar">j</span><div><strong>@jodie.attempts</strong><span>is glass drop actually worth the £62 or is it tiktok hype</span></div></div><Flower className="stage-flower flower-left"/></aside><main className={`app-shell view-${view}`}><div ref={screenTop} className="screen-top"/><header className="app-header"><Brand/><div className="profile" aria-label="Maya's profile">m<span/>{inbox.queue.filter(item => item.source === 'live').length > 0 && <i className="live-count" aria-label="New live replies">{inbox.queue.filter(item => item.source === 'live').length}</i>}</div></header>{inbox.error && <div className="connection-error" role="alert"><span>{inbox.error}</span><button onClick={inbox.reload} aria-label="Retry loading inbox"><RotateCcw size={16}/></button></div>}
  ...
  </main><aside className="desktop-note" aria-hidden="true"><div className="ghost-reply">yes — layer it under the serum, especially in winter<span>maya’s answer · used 14×</span></div><div className="note-line"/><p>still you.<br/>just with a little<br/><em>more time.</em></p><Flower className="stage-flower flower-right"/></aside>{shortcuts && <Shortcuts onClose={() => setShortcuts(false)}/>}</div></MotionConfig>;
}
